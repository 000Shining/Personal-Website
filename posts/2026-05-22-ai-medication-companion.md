...
```
           Now (0-3月)              Next (3-6月)            Later (6-12月)
    ┌─────────────────────┬─────────────────────┬─────────────────────┐
    │ 验证核心假设          │ 核心闭环 + 子女端      │ 硬件协同 + 生态扩展    │
    │                     │                     │                     │
    │ ██ 用户研究+Wizard   │ ██ App开发+提醒引擎   │ ██ 硬件量产+公开上线  │
    │ ██ 多路径录入实验     │ ██ 子女端异常告警      │ ██ 医生端+问诊对接    │
    │ ██ 双端提醒覆盖实验   │ ██ AI识别+语音问答     │ ██ NFC贴纸方案落地    │
    │ ░░ App设计探索       │ ░░ 药盒ID设计          │ ░░ 社区卫健系统对接    │
    │ ░░ 药盒BOM调研       │ ░░ 医生端需求访谈      │ ░░ 保险/健康管理合作   │
    │                     │                     │                     │
    │ ← 用户研究持续贯穿 → │                     │                     │
    │ ← 硬件探索持续贯穿 → │                     │                     │
    └─────────────────────┴─────────────────────┴─────────────────────┘
...
```
用户招募 ──→ 录入实验 ──→ Go/No-Go ──→ App架构 ──→ 提醒引擎 ──→ 内测 ──→ 公开上线
     └──→ 提醒实验 ──┘                         └──→ 子女端(并行) ──┘       └──→ 硬件(并行)
                                                     └──→ AI识别(串行) ──→ 语音问答
```
